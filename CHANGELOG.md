...
- **DEVON and Hermes audit of 2026-09-02** (`docs/devon/SYS_OPS_devon-hermes-agent-audit_v1_2026-09-02.md`, PR #110) and the fix PRs that followed it, each merged after a fresh critic pass and green CI:
  ...
  - this fix (audit item 16): `app/security/jwt.py` signs and verifies with PyJWT instead of python-jose, unchanged externally (same algorithm, same claims, same TTL, same 401 on an expired, tampered, wrong-key or malformed token, proved by new tests and by decoding a jose-issued token with the new code); `python-jose` and its sole dependency, `ecdsa` (`PYSEC-2026-1325`), are gone from `requirements.txt`, `PyJWT==2.13.0` is pinned in its place, and the `--ignore-vuln PYSEC-2026-1325` clause is gone from both `pip-audit` invocations in `.github/workflows/ci.yml` because a fresh audit of the new pins finds nothing to ignore
  - this fix (audit item 17): `requirements.txt` is now generated, never hand-edited, the pip-compile --generate-hashes output of a new `requirements.in` that carries the same 21 direct pins, expanded into the full 58-package closure with every direct and transitive dependency pinned and hashed; `Dockerfile.api`'s pip install line now passes --require-hashes, so a Railway build either reproduces this exact set or fails outright instead of resolving transitives fresh. Verified in a clean virtualenv (identical package set to the unhashed file, confirmed by diffing pip freeze output, and a corrupted-hash negative control that the same install correctly refused) and with pip-audit (no known vulnerabilities against the new file); `deploy/soul/requirements.txt` is untouched, as the item is scoped to the Railway API image
  - the fix PR 15 critic's follow-up wrapped `services/knowledge/pipeline.py`'s cross-encoder completion provider in `MeteredProvider` but left it permanently inert, since it imported a factory function that did not exist and called the provider with a shape that did not match its contract; `create_completion_provider` now exists in `services/intelligence/providers/factory.py` and `_llm_judge_score` calls the real `AIProvider.complete(CompletionRequest(...))` contract, so the judge runs a genuine completion, still capped, when `POST /api/v1/knowledge/query` is served with a real provider configured instead of always falling back to the offline lexical score

### Changed
- **Claude Code on the web can reproduce CI without setup** (PR #132): `CLAUDE.md` carries the project memory (the six CI jobs and how to reproduce each, the governance invariants, what a new migration touches, the ship discipline), and `.claude/hooks/session-start.sh`, registered as a SessionStart hook, installs the pinned Python closure, the locked pnpm workspace and a PostgreSQL 16 with pgvector cluster at `/var/lib/pgtest`, then writes the api and engine job environment to `CLAUDE_ENV_FILE`. Remote sessions only, idempotent, database stage best effort so a session always starts. Verified on a container the hook prepared: standalone 137 passed, engine 25 passed, the full api suite 1318 passed, ruff clean, web typecheck and production build clean
- **Deployment records written back from a live read** (2026-09-04): `list_teams` reports the Vercel account on the Pro plan, so the deploy-readback skill no longer states the free plan's 100 per day as the current limit and keeps that cap as the history that explains the `ignoreCommand`; the skill gains the account block as a third failure mode distinct from a correct skip and from the daily cap, including its signature (no deployment record of any kind is created while blocked) and how to verify it cleared (a record created at all, `CANCELED` included); the ecosystem spec's read back table stands at 2026-09-04 against `edaf03e` with the 2026-08-26 table kept as superseded; `scripts/estate_reconcile.py` pins the retired free plan sentence as a tripwire. Both Vercel skips were verified by hand against each project's own `vercel.json` rather than trusted
