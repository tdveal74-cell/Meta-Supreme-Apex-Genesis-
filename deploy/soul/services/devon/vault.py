...
WEBHOOKS = {
    ...
    "devon-hears": {
        ...
        "open_ruling": (
            ...
            "same value into a HEADER AUTH credential named DEVON Service Key, "
            "name x-devon-key, then picks it on that node. The key is at least "
            "24 characters or the door refuses it the same way it refuses an "
            "unset one; secrets.token_urlsafe(32) gives 43. "
            "The node briefly asked for httpTemplatedCustomAuth instead, which "
            "was wrong and is corrected. The SDK validator refuses to CREATE a "
            "new httpHeaderAuth credential from workflow code and steers to the "
