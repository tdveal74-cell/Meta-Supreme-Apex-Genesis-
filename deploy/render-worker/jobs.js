 ...
 * 1 over `ramp` seconds, holds 1 across the gap, and ramps back down. Windows
 * are rejected if they overlap, so at most one term is non-zero at any t; the
 * outer max/min clamps the result anyway rather than trusting that.
 *
  ...
  duck_mix: {
    build(p) {
      ...
      const argv = ['-y', ...QUIET, '-i', nar, '-i', bed, '-filter_complex', filter,
                    '-map', '[mix]', '-ar', '48000', '-c:a', 'pcm_s24le'];
    ...
    build(p) {
      ...
      shots.forEach((s, i) => {
        ...
        if (still) {
          ...
          clips++;
          trims.push(`[${i}:v]trim=start=${IN}:end=${OUT},setpts=PTS-STARTPTS,` +
      ...
      return {
        ...
        parse: () => ({
          ...
          fps,
          normalized_to: `${W}x${H}`,
          expected_duration: Number((planned - xf * (shots.length - 1)).toFixed(6)),
        }),
      };
    },
  },
