...
- I serve the Loop HUD same-origin at app.main GET /console. Remember / file a ruling / add a task / start a project / log a thread propose on this host with the CurrentUser JWT. PLATFORM is not required there. CONSOLE_TOKEN is only for soul-host recall on devon-soul.vercel.app. That Vercel host has no Postgres, production SHA is 57fdddb (the PR #111 merge, deployment dpl_8bdZuGfJSB9sncUMoge1FUUSiUXN READY as production on 2026-09-02; every later main merge changed no file under deploy/soul and was ignored by the path filter), and I do not pretend localStorage is memory.
