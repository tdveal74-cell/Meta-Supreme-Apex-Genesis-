...
WORKFLOWS = {
    ...
    "OS 29 Platform Policy Sensor": {"id": "7WyIarNoJa2irx2r", "state": "active since 2026-09-08, daily 06:00 America/New_York, timezone pinned the same day, nine sources watched once the X Original Content Rewards successor was added, activeVersionId 138e7ceb; the Firecrawl fallback on Gateway credits runs with a custom body (waitFor 15000, onlyMainContent false, maxAge 0, location US en-US, rawHtml dropped) because the node defaults captured hollow policy sections on Meta Content Monetization from the day the sensor was built and its documented two day default cache could serve a daily sweep a stale copy, both found and fixed 2026-09-08; comparison is a block level diff with a two flip volatile rule, a completeness rule refuses a hollow capture on both paths, AI Verdict is append only"},
