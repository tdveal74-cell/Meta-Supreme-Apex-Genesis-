...
DATE: 2026-09-16
DECISIONS: Tee ruled on an inline card to convert Data Table resolution from name to id. The conversion is additive rather than a rename: tableId and __table keep their values and their meaning, a tableRef and __tableRef carry the id beside them, and only the locators move, so a missed node keeps working on the old path and a locator pointed at an unset ref fails loudly instead of returning zero rows. DEVON Pipeline Watchdog was converted alongside V5 without a separate ruling, because it is the only other ACTIVE workflow with by-name locators and two literal ids close the active surface completely. TQO FINAL V5 - ADAPTER TEST was deliberately NOT converted and is handed back: it is inactive, it is a copy of V5, and activating it as it stands reintroduces the hazard in full. Everything went in as a draft first, in six stages, because a draft does not touch the active version and that removes the schedule entirely.
...
OPEN: TQO FINAL V5 - ADAPTER TEST still carries 29 name mode locators and is Tee's call; it is inactive, so nothing is exposed today, and scripts/n8n_table_id_conversion.py converts it mechanically. 15 more sit in DT Bootstrap Tables, S5 Seed and three throwaways. Throwaway 8dsCOEDaG7Tnu5Sb cannot be archived from here because MCP access is off on that workflow, so it needs archiving in the n8n UI. The conversion has not yet been observed in a real scheduled V5 pass; the next is 13:00Z and it has not run. The two other rulings from this session, building the LiveKit room publisher and making an empty balance 400 surface its real body, are not started. PR #248 states that test_presence_service.py:106 pins livekit_configured false for production; it pins the test environment's value and that correction is not yet filed anywhere but here.
STATUS: Done and published on both active workflows, proven four ways before publishing and read back byte for byte after. The durable fix Tee ruled is live on every workflow that runs. The remaining 44 name mode locators are all inactive and the largest block is handed back.
