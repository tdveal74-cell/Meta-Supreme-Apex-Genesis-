...
- Gumroad's documented scopes include `view_sales`; whether an app-page token can be
  ...
  says rotate monthly if no scope picker exists. Which stands is Tee's to say.
...
OPEN: view_sales inferred not proven; V5 published with schedules disabled, activeVersionId 73efec8d, each schedule re-enabled as its own named act when Tee can watch it; the location pin has two fresh samples one each way and the 10:00 UTC firing is the third; the Firecrawl failure path emails nothing and needs a ruling; the Gumroad rotation conflict between the record and the card needs Tee's word; PR #167 carrying this amendment waits on Tee's merge
