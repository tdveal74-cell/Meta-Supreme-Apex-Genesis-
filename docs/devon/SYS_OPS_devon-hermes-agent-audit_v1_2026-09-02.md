...
Owner to fix: classify by resolved path against the root for every argument, deny `/proc`, `/sys`, dotfiles and anything outside the root on the READ lane. Status 2026-09-02: fix PR 5 judges every path-like argument on the read lane, option values such as --git-dir= and rev:path forms included, after symlink resolution: outside the operator root, under /proc, /sys or /dev, or naming a dotfile in any component fails closed to human approval with the path in the reason. The fresh critic showed the first cut still printed the process environment through `ps eww`, because ps takes no path argument and reads /proc/<pid>/environ itself; ps left the read-only set and waits for the human. /proc, /sys and /dev are matched by path component, not string prefix, so an operator root under /devon is not caught. Named friction: reads of ordinary dotpaths such as .github/ and .gitignore now wait for the human too. Verified by test_operator_bridge.py, including the audit's own `cat /proc/self/environ` and the critic's `ps eww`.
...
Owner to fix: acquire the lease first, then check orphans filtered by execution generation. Status 2026-09-02: fix PR 7 takes the execution lease first, so a run that lands while another worker holds a live lease is told TaskExecutionBusy and touches nothing, and checks orphans only with the lease in hand. No generation filter is needed for correctness: intent and receipt writes are fenced by the live lease token and generation, so while this worker holds the lease no other worker can be between an intent and its receipt, and every receipt-less intent belongs to a generation that lost its lease. The refusal fails the task under the held lease, closes the run row through the fenced failure path (so the same idempotency key replays the refusal), and releases the lease. Verified by test_devon_lease_loss_crash_matrix.py: the audit's own run (A mid-effect, B told busy, A's lease and row untouched, A's receipt lands) and the six-worker orphan race, where every attempt refuses as busy or ambiguous and none runs. The fresh critic then showed two windows in the first cut: a refusal landing after the lease had expired still wrote the task row with an unfenced upsert, and an exception in the check itself left the lease claimed until expiry. The check now runs inside the failure path that releases the lease, the refusal's task write is fenced on the lease token (a lost lease writes nothing and is reported as lost), and the six-worker test asserts that each ambiguous refusal closed a failed run row, which the old code never did.
...
Owner to fix: write the ledger rows first inside the request transaction and spend the approval last, or make the spend part of the same transaction. Status 2026-09-02: fix PR 8 takes the first option. ACTION_STARTED, the artifact, ARTIFACT_CREATED and LEARNING_CAPTURED are written and committed on the request session before the approval is spent; the spend is the last thing before the effect; everything after the spend (the connectors, ACTION_COMPLETED, the receipt and its commit) runs inside a failure recorder that appends ACTION_FAILED (consumed true, the error named, secrets redacted) in its own commit and answers 502 naming the intent; a connector that refuses is a completed capture with the refusal written on ACTION_COMPLETED, not a failure; the refusal of a spent approval names the intent and its terminal event (ACTION_COMPLETED, ACTION_FAILED with the error, or ACTION_STARTED with no terminal event); a spend that fails appends ACTION_FAILED (consumed false); LedgerConflict answers 409 on the soul routes. The fresh critic's conditions (recorder on the reachable path, the CONSUMED refusal pointing at the ledger) were applied before merge. Commits of one request are serialized with a transaction-scoped advisory lock on the intent, and the second commit finds the first one's ACTION_STARTED and is refused 409, so the effect runs once. Named residual: a process that dies between the rows commit and the spend leaves ACTION_STARTED with no terminal event and an unspent approval; the next commit fails closed with that reason and the capture is re-proposed. Verified by test_devon_knowledge_loop_durability.py: the audit's failing-effect run, the die-before-spend run, and two concurrent commits; against the old code the failing effect escapes as a 500 with no ledger row. Status 2026-09-03: fix PR 14 closes the row-state half the fix PR 8 critic filed as F4. The ledger's approvals row stopped at 'approved' and stayed there whatever became of the approval; migration 018 widens the constraint to carry 'consumed' and `ledger.settle_approval` now writes the three settlements at the three places that already know: the commit that spends (on the failure path too, since the approval is spent either way), the decide route that refuses, and the store sweep that expires. Verified by test_devon_ledger_row_states.py, seven tests.
...
Why it matters: AUDIT.md section 3 item 2 asks reviewers to attempt exactly this attack and says the preview and the write cannot diverge because they are the same object. They diverge the moment the definition changes. Status 2026-09-02: fix PR 10 makes them one object again. PATCH refuses a definition change with 409 while a run of the workflow awaits approval (name, description and status changes still pass). When a run pauses, it seals a sha256 of the rendered pending payload (step id, step type, rendered preview) in its metadata, the pending view returns that hash as payload_sha256, and approval re-renders from the live definition and refuses 409 when the rendering no longer matches the seal, so a definition swapped by any path, PATCH or not, cannot execute under the old approval. The approver may also name the hash it saw (expected_payload_sha256), and a mismatch is refused. Runs that paused before this change carry no seal and are not checked against one; the PATCH guard still covers them. Verified by test_devon_workflow_gate_binding.py: the audit's swapped-text and retyped-step cases through a direct definition write behind the gate, the PATCH refusal, and approval naming the right and the wrong hash. The fresh critic's conditions and two Codex review findings were applied before merge: decisions are refused for any step other than the pending one (a later gate renders its own preview); a seal mismatch is logged with the run and both hashes; the pending view carries the seal and names divergence; the workflow's project is part of the sealed payload; a rejection always closes the run, even when the definition no longer parses; PATCH refuses only an actual definition change; expected_payload_sha256 must be lowercase hex; a definition that stops holding behind the gate answers 409, not 500.
...
No invite, flag or policy. `billing.py` limits are imported only by `standalone_api.py`. On the deployed API any internet user can mint a tenant, run councils against Tee's provider keys, and raise cards into the queue H1 already shows is shared. This is a policy gap, not a code defect, and needs Tee's ruling. Status 2026-09-02: registration was closed behind DEVON_REGISTRATION_KEY in PR #111 (57fdddb). Fix PR 15 caps provider spend per account, at the one chokepoint every lane passes through and never per route: councils, agent turns, workflows, the planner, enrichment and embeddings all take their provider from app.services.intelligence, which now returns it wrapped in MeteredProvider (MeteredEmbeddingProvider for embeddings). Before each call the wrapper reads the account's provider_usage row for the UTC day and refuses with a typed 429 (code provider_spend_cap, Retry-After set, the detail naming the cap, the day's usage and the UTC reset time, nothing sent to the provider) once input plus output tokens have reached PROVIDER_DAILY_TOKEN_CAP (default 500,000, about 25 full council runs; 0 disables the refusal and the ledger still records); after each call it upserts what the provider reported in its own short transaction on the engine, so the record survives a request that fails afterwards, including a session that rolls back. The account comes from a request-scoped context variable bound by the auth dependency and cleared by a middleware when the request ends, so no call site changed; the schedule dispatcher binds the workflow's owner for a scheduled run, and work with no account in hand (startup jobs, service-layer calls outside a request) is counted and capped under the "system" bucket, so an attribution gap fills up and becomes visible instead of draining the key unbounded. The ledger is the provider_usage table (user_id, usage_day, calls, input_tokens, output_tokens, updated_at), added by migration 017_provider_usage and schema 017_provider_usage.sql; the revision chains after 015 because the 016 that fix PR 14 was to add has no commit in the repository. billing.py was left alone: its limits are a monthly plan catalog with no persistence and no user-to-plan mapping, a different object from an operator's flat daily ceiling. Named residuals: the check runs before each call and the record after, so concurrent calls that all pass the check can overshoot by one completion each; a cap crossed mid-council refuses the next call of the same request, after the calls that ran were paid and recorded; attempts the inner provider retries on a failed call report no usage and are not recorded. Verified by test_devon_provider_spend_cap.py: an account at the cap gets 429 and the mock provider's call count stays at zero, rows increment per call by the tokens reported, a second account is unaffected, cap 0 disables, the system bucket is capped, the dispatcher spends under the owner, and the row survives a message request whose session rolled back.
...
- ELC-02 medium `app/services/agent_effect_receipts.py:124`: receipt and intent fence refusals are bare RuntimeError and surface as 500, not the typed 409 the crash-matrix method requires. Status 2026-09-02: fix PR 7 types them as EffectFenceRefused (a RuntimeError subclass, so existing catches hold) and the run route answers 409 for it alongside the other lease refusals.
...
- ELC-08 low `app/services/agent_tasks.py:384`: the orphan check precedes the idempotency lookup, so a legitimate replay of a completed key is refused. Status 2026-09-02: closed by fix PR 7 as a side effect of the ordering: the idempotency lookup now runs inside the claim, before the orphan check, so a completed key replays with an orphan present (the critic measured 200 with Idempotent-Replay true) and only a fresh key is refused.
...
- DVC-11 medium `docs/devon/SYS_OPS_devon-hermes-stack-status_v2_2026-08-25.md:56`: re-edited on 2026-09-01 and still says the deployed database is at Alembic head 010. It is 013. Status 2026-09-02: 015 since fix PR 1 (#111); the doc now reads head 015, dated, and both the retired and the standing sentence are pinned against the migrations directory (fix PR 12).
- DVC-12 medium `docs/GAUNTLET.md:39`: FLAGSHIP, COMPLETION and GAUNTLET say Hermes expansion is in-memory and not durable. The HTTP routes persist schedules, proposals and links. Only the runtime tool path is in-memory (H6). Status 2026-09-02: closed by fix PR 3 (#114); `docs/GAUNTLET.md:43` dates the runtime tools durable.
- DVC-13 medium `docs/GAUNTLET.md:44`: calls FKR schema only while a hybrid query route is mounted. Given H9 the route fails on every request, so the doc is closer to the truth than the code. Status 2026-09-02: the premise changed with fix PR 9 (#119), the query route answers; `docs/GAUNTLET.md:44` now dates that the route is mounted and answers while the product stays schema-only (fix PR 12).
...
- Agent tooling committed to the repo: `.claude/settings.json` enables three community plugins with no version pin, and `.agents/skills/unlazy` vendors 37 files including a Stop hook installer. `skills-lock.json` hashes SKILL.md only. The hook is not installed today. Nobody has said whether this is intended. Status 2026-09-02, per ruling 3: fix PR 6 removes `.agents/skills/unlazy` (37 files), the `.claude/skills/unlazy` symlink, `skills-lock.json` and the four unlazy lines in .gitignore, and pins the three plugins through a repository-local marketplace, `.claude-plugin/marketplace.json`, registered from `.claude/settings.json` as a directory source. Each entry carries the exact upstream commit the community marketplace resolved it to at its commit a727be1 (agentic-guardrails 8313dcf, backend-security-skills ee05fc0, test-generator 4698df2); the community marketplace itself keeps moving with every upstream bump, and marketplace sources accept a ref but not a sha, which is why the pin lives in a local marketplace rather than on the community source. Measured: `claude plugin install` of all three from the local marketplace under a throwaway HOME recorded gitCommitSha equal to each pin, and the fresh critic reproduced it with cache trees byte-identical to the pinned commits while agentic-guardrails upstream had already moved on. Per machine, once: run `claude plugin install <name>@meta-supreme-pinned` for the three names from the main checkout of this repository (a worktree resolves the directory source against the main checkout, and a session that has not trusted the folder registers nothing), and uninstall any `<name>@claude-community` copy, because the critic showed a leftover community copy loads beside the pinned one and its agent wins. To bump a pin: change the sha in `.claude-plugin/marketplace.json` as its own PR, then on each machine uninstall and reinstall that plugin; the critic measured that `claude plugin update` reports "already at the latest version" when the plugin's own plugin.json version string is unchanged, which is the case for agentic-guardrails (1.0.0) and backend-security-skills (0.2.2). The pin is visible as gitCommitSha in `~/.claude/plugins/installed_plugins.json`; `claude plugin list` shows only the plugin.json versions. Deployed 2026-09-02: Railway deployment 5d948461 of merge b6a48c2 (fix PR 6) reached SUCCESS at 20:35:15 UTC with "Application startup complete"; no runtime change was expected and none was seen.
...
3. Ship `014_artifact_body.py` executing the existing idempotent SQL, add the 004 `content` column to a migration the same way, update the two ci.yml presence loops and the head assertion, and add one CI step that diffs information_schema between the Alembic build and the SQL build and runs the ledger and knowledge suites against the Alembic build. Closes C3, H10, H11. Then run the one production query in C3 and record the answer, dated. Status 2026-09-03: closed by fix PR 14. `014_artifact_body.py` shipped with fix PR 1; migration `018_schema_convergence` adds the 004 `content` column to the Alembic chain and widens `ck_approvals_state`; the two presence loops and the head assertion name it; and one CI step builds both databases, dumps their shape with `scripts/schema_shape.py` and fails on any difference, then runs the ledger, knowledge-loop, ledger-state, knowledge and FKR suites against the Alembic build with `TEST_SCHEMA_PREBUILT=1` so the mirrors cannot paper over a divergence. Measured before the fix: the two builds differed by exactly one line, `knowledge_items.content` present in the SQL build and absent from the Alembic build, which is the build Railway deploys, so every ingest path wrote a column production did not have. Measured after: 681 shape lines, no difference, Alembic head `018_schema_convergence`.
...
13. Rule on registration policy (invite key or a closed-by-default flag) and on per-tenant provider spend. H15 is a decision, not a patch. Status 2026-09-02: ruled and shipped. Registration is closed behind DEVON_REGISTRATION_KEY since PR #111; Tee ruled that the spend cap ships, and fix PR 15 enforces PROVIDER_DAILY_TOKEN_CAP per account per UTC day in the metered provider wrapper that every lane takes from app.services.intelligence, recorded in the provider_usage table added by migration 017_provider_usage, refused as a typed 429 before anything reaches the provider.
14. Write back the records: production SHA in FLAGSHIP, COMPLETION and GAUNTLET, retire `meta-supreme-web` from DEPLOY.md, the deploy-readback skill and the ecosystem spec table, Alembic head in Hermes v2, the dispatcher command, RUNBOOK paths, and the CHANGELOG dispatch claim. Run `scripts/estate_reconcile.py` afterwards and pin the corrected sentences. Status 2026-09-02: fix PR 12 writes back every record named here (production SHA 57fdddb in FLAGSHIP, COMPLETION and GAUNTLET, dated; `meta-supreme-apex-genesis-web` in DEPLOY.md, the deploy-readback skill and the ecosystem spec table; Alembic head 015 in Hermes v2; `python dispatch.py` in OPERATING, RUNBOOK, the CHANGELOG and the dispatcher docstring; RUNBOOK paths to `app/services/workflows.py`, `app/api/v1/workflows.py` and `test_workflows_api.py`) and pins each retired sentence in `scripts/estate_reconcile.py` DOC_CLAIMS as a tripwire: the old sentence coming back reports DRIFT, and the standing Alembic head and cron entrypoint are checked on every run: the head against the migrations directory at the commit of the newest successful Railway deployment (UNVERIFIED without a Railway read, never from the source tree alone), the entrypoint against both the checkout and the COPY lines of Dockerfile.api, which now ships dispatch.py so the documented `cd /app && python dispatch.py` runs inside the image. Codex on the first cut found all three (the entrypoint missing from the image, the head certified from the source tree, and two historical docs still naming the old module), applied before merge.
15. Add `pip-audit` and `pnpm audit` to CI, and pin requirements. Status 2026-09-02: fix PR 11. Measured before the change: pip-audit on requirements.txt found one advisory, ecdsa 0.19.2 (PYSEC-2026-1325, pulled in by python-jose, no fixed release published); pnpm audit found four high and two moderate advisories, all transitive through next (postcss, nanoid, sharp). requirements.txt pins its 21 direct dependencies to the versions the full suite passed against on this date (transitives still resolve at build time; hashes are item 17), the duplicate httpx line removed; the root package.json carries pnpm overrides postcss>=8.5.23, nanoid>=3.3.18 and sharp>=0.35.0 and the lockfile was regenerated, after which pnpm audit reports zero; a dependency-audit CI job runs pip-audit on the pinned file (with the ecdsa advisory ignored by id, dated, until a fix ships) and pnpm audit at the moderate level on every push and pull request. The fresh critic's conditions were applied before merge: the audit job runs on Python 3.12 (the Dockerfile.api interpreter), it audits deploy/soul/requirements.txt as well (the live devon-soul host), the nanoid override stays on the 3.x line, and the ecdsa ignore carries an owner.
16. Replace python-jose with PyJWT in `app/security/jwt.py` (one import, one configurable algorithm). python-jose pulls ecdsa, whose maintainers will not fix PYSEC-2026-1325, so the CI ignore never expires on its own. Status 2026-09-02: open, filed by the fix PR 11 critic; its own PR. Status 2026-09-03: fix PR swaps the import (`from jose import JWTError, jwt` to `import jwt`) and the one catch clause (`jose.JWTError` to `jwt.PyJWTError`, PyJWT's own base exception, which every error `decode()` can raise is a subclass of, matching the old catch-all); the algorithm, the claims (sub, exp, iat, type), the TTL and the decode outcome for an expired token, a tampered signature, a token signed with the wrong key and a malformed token are all unchanged, each proved by a new test in `test_security.py`. A token hand-encoded with jose's exact old call shape decodes correctly under the new PyJWT code, and the reverse holds too, so tokens already issued by the running system keep working. `requirements.txt` drops python-jose and pins `PyJWT==2.13.0`; ecdsa was never pinned directly, had no other dependent, and is now gone from the resolved set along with python-jose, confirmed by a fresh pip install and a pip-audit run that finds no known vulnerabilities, so the `--ignore-vuln PYSEC-2026-1325` clause is removed from both pip-audit invocations in `.github/workflows/ci.yml`. `deploy/soul/requirements.txt` never imported jose and is untouched. The full suite (1309 tests) and ruff pass on the new code.
17. Compile the Python dependency set with hashes (pip-compile --generate-hashes, or a constraints file) and install with --require-hashes in Dockerfile.api, so the Railway image is byte-identical to the audited set; today only the 21 direct dependencies are pinned and transitives resolve at build time. Status 2026-09-02: open, filed by the fix PR 11 critic; its own PR. Status 2026-09-03: fix PR compiles `requirements.in` (the same 21 direct pins, unchanged) into `requirements.txt` with pip-compile --generate-hashes, from the pip-tools package named in the item; two independent compiles against Python 3.12, one with a fully cleared pip cache to force a real PyPI resolution, produced a byte-identical 58-package closure, and a hash spot-check against the PyPI JSON API confirmed the file's sha256 values match the real published wheels and sdists, including for ast-serialize, an unfamiliar transitive of mypy that turned out to be a genuine mypyc-adjacent package rather than a confusion risk. `requirements.txt` is now generated output and is never hand-edited again; `requirements.in` carries the regeneration command in a comment. In a clean virtualenv, pip install --require-hashes -r requirements.txt installed the identical 58 package versions the unhashed file installed, confirmed by diffing both pip freeze outputs, and a negative control that corrupted two hashes on one package made the same install fail with pip's own tamper error, so the flag is verified to do real work rather than sit inert. `Dockerfile.api`'s install line now runs pip install --no-cache-dir --require-hashes -r requirements.txt. This sandbox's own egress policy returned a 403 on the docker build step's python:3.12-slim base image pull, confirmed at the proxy status endpoint as a policy denial rather than a transient failure, so no full docker build ran here; the pip install line Dockerfile.api runs was instead reproduced directly against Python 3.12.3, the interpreter the image ships, and it installed clean. CI's own Railway container contract job builds this Dockerfile on every push and remains the real integration test for this change. pip-audit against the new file still reports no known vulnerabilities, and the full suite (1309 tests) and ruff pass unchanged, since no package version moved and only hashes were added. `deploy/soul/requirements.txt` is untouched, as scoped.
...
| .agents/skills/unlazy and the unpinned community plugins | Remove unlazy, pin the three plugins by version | Small separate PR, no runtime effect. Done in fix PR 6: unlazy removed, plugins pinned by upstream commit in `.claude-plugin/marketplace.json` |
...
| The C3 production query | Run it now, read-only, before the 014 migration ships | Not captured. On 2026-09-02 the query was run on the EditForge VPS EDITFORGE-HOST, which holds no DEVON database (the DEVON database is the Postgres service in the Railway devon-api project), and Tee then ruled "Merge" without the before-state. PR #111 merged as 57fdddb. Railway deployment 782b095d (SUCCESS, 08:35 UTC) logged `Running upgrade 013_approval_consumption -> 014_artifact_body` and `Running upgrade 014_artifact_body -> 015_devon_approval_owner`, so production now carries artifacts.body, artifacts.kind and devon_approvals.owner_id at head 015. Whether body and kind were missing before that deploy is unknown and stays unknown: Alembic surfaces no Postgres notices, so the log's silence proves nothing either way |
