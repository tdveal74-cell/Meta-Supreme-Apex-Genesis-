...
16. Replace python-jose with PyJWT in `app/security/jwt.py` (one import, one configurable algorithm). python-jose pulls ecdsa, whose maintainers will not fix PYSEC-2026-1325, so the CI ignore never expires on its own. Status 2026-09-02: open, filed by the fix PR 11 critic; its own PR.
