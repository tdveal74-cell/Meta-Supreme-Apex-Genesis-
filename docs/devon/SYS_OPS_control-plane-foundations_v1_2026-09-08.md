...
DATE: 2026-09-08
DECISIONS: none were ruled during this arc; the four the spec put to Tee were ruled 2026-09-09 on a card and are recorded in SYS_OPS_rulings-review-gate-and-receipt-canon_v1_2026-09-09; the two merges were made on stated authorization from Tee, which the repository itself cannot show
FINDINGS: four gauntlet passes, three returning QUARANTINE and the fourth PASS-WITH-CONDITIONS at security 5 and mean 4.5; the hash broke on legitimate floats until it hashed what PostgreSQL stores; DELETE was open on events, receipts and intents in turn; the shipped stack ran the API as the database superuser; a temporary table's trigger deleted a receipted history at trigger depth 2; the runtime role held DELETE on users; the intent row's owner, channel, statement and effect flag were unbound until the opening event hashed them; a 400 digit wire number killed the socket; the GitHub timeline for PR #176 carries no reviewed event
OPEN: asymmetric session tokens and a second receipt law were ruled 2026-09-09 and are queued as their own arcs, neither built here; chain heads are not anchored outside the database; the owned likeness and the secret vault stay with Tee; forty of the fifty two status docs carry no receipt at all
STATUS: merged to main as 2b09dbd (the arc) and 1b13724 (this close-out), CI green on both merge commits, Alembic head 019_event_hash_chain, the presence service and the /presence surface built and never exercised against a live LiveKit, Cartesia or Cerebras endpoint
