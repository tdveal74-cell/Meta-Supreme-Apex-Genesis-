...
store, and its render worker has one recorded smoke among fourteen
successful manual runs of the VPS TSWS 00, none recorded as moving a sample. The VPS runs no DEVON organ and carries a second, diverged copy of
...
Vercel log store already holds twenty such lines from this morning. The
record disagrees with itself in fourteen places, six of them same date docs
the filenames cannot order.
...
in five files. `ci.yml` is the only file that runs on every push to `main`;
`render-worker-ci.yml` runs on a push that touches `deploy/render-worker/**`
and the other three run on pull requests only, so the five direct commits
pushed on 2026-09-14 and 15, all under `apps/web`, got the Python suite
...
reflection half of Build 13 was therefore silent from about 2026-09-13T12Z
...
| Live State Ledger on Postgres, migrations 012 and 019, provenance card | Railway api, `/control` | head `019_event_hash_chain` in the tree, and the api's pre deploy command is `alembic upgrade head` on `95fbdd5`, so the deployed head is inferred, not read; provenance card read it on 2026-09-09 | ready with conditions: chain heads not anchored outside the database |
...
| Event Bus and receipts | n8n Cloud | 24 trace entries on the live row of the 2026-09-12 job (23 in the 09-12 record, the 24th LEARNING_CAPTURED on 09-13) and a receipt | ready with conditions: the bus itself unread since 2026-09-06 |
...
| n8n Cloud, executor of record | n8n Cloud | 49 workflows, 41 active, 12 timer runs and one manual Table Reader run today | ready; the vault mis-states three entries |
...
1. The VPS action gate proxy logs a secret prefix. On `origin/main`,
   ...
   because PR #219 is the Soul arc and a one line change to a lane that has
   no record yet is Tee's call.
...
3. TQO FINAL V5 is active on both instances, with seven live webhook
   ...
   lines 114 to 115 say the VPS copy is inactive with zero triggers and the
...
4. The CI count is stated three ways: the steward skill says five jobs plus
   ...
   files hold nine. Only `ci.yml` runs on every push to `main`, so a direct
   push under `apps/web` gets the Python suite and nothing else; `test_devon_integrity.py` inside
...
8. The record disagrees with itself in fourteen places. The ones that
...
- 2026-09-06: register or archive the ten DEVON named workflows absent from
  the vault; fix the Notion Buffer Drain's eight Area list and its 1900
  character truncation; read the real n8n execution cap.
- 2026-09-08: anchor chain heads outside the database.
- 2026-09-09: the chat voice on `/devon` unheard; nobody has filed a capture
  through keep; the console mute button toggles over a refusing Voice; the
  receipt canon backlog of forty five exempt docs awaits a ruling.
- 2026-09-10: Tee opens `/control` on a phone and looks; the stray Railway
  service `Meta-Supreme-Apex-Genesis-` `6fb2feb3` with a 57 change staged
  patch, not among the four services of project `devon-api` today and
  possibly in another project, unverified; seven trigger sweep routes gate
  on prose; decide which instance owns the schedules; rotate the ElevenLabs
  key found as a literal in an archived sub-workflow; the two
  unauthenticated GET link triggers on V5.
- 2026-09-11: no V5 lane has run end to end on Gateway credits because every
  schedule is disabled; the four step render worker acceptance suite on real
  media.
- 2026-09-12: where EditForge runs; no human can sign in to
  ...
  snapshot has no scheduled run.
- 2026-09-15, new: Tee sets the two soul variables on Railway api and runs
  the four read back steps; merge PR #219 so `main` goes green and the dock
  stops rendering a failed read as recall off; remove the proxy debug
  logging; write a record for the VPS action gate; set the five execution
  cap anchor variables from the usage page; reconcile the CI count in the
  steward skill; move the reflection to a standalone Routine as beat 84
  asks.

## What could not be measured here

Direct reads of `railway.app`, `vercel.app` and `ops.editforge.online`, all
refused by the egress policy, so the presence health payload and any direct
probe of the ops gateway are unverified; the gateway is known to answer only
from the status codes the proxy forwarded. The `devon-soul` project's
variable names. Soul Committer, Capture Webhook and Approval Queue execution
health, because those three persist no executions by design; their tables
were read instead. The Face's instance default for saving successes.
`devon_soul_commit_log` and `devon_driver_log`, not read; soul REVERTED 1
comes from the heartbeat vitals. Whether either expired pending card carries
a decision that never landed. The diff behind V5's version drift,
`bde7ddec` in the vault against `0c1f7068` live. The deployed Alembic head,
inferred from the pre deploy command and the 09-09 record. The twelve name
gap between the 29 variable names the service config lists on the api and
the 41 an earlier variable read counted. Who ran the manual Table Reader
execution at 05:54Z. And every external poster's target, which is inferred
from the vault and was not read.
