...
export function KnowledgeGraphPanel() {
  ...
  const view = useMemo(() => {
    ...
    return {
      ...
      // BOTH arguments, and the flag is the load bearing one. Passing only the
      // name made the route's "unavailable" provider read as real, which is the
      // state a default start-devon.sh launch produces. counts being null makes
      // this undefined, which classifyProvider reads as "unknown", not "real".
      trust: classifyProvider(graph.provider, graph.counts?.embedding_provider_simulated),
      // Whether the radius means anything in THIS payload. With no edges every
      // degree is 0 and every mark lands on one ring, so the description must
      // not promise a shape the picture does not have.
      radialDegree: radiusEncodesDegree(graph.nodes),
    };
  }, [phase]);
  ...
  return (
    <div className="space-y-4">
              ...
              <div className="overflow-x-auto">
                ...
                >
                  ...
                  <desc id="knowledge-graph-desc">
                    {`Each mark is a knowledge item, placed on a ring grouped by source. ` +
                      (view.radialDegree
                        ? `The most connected items are pulled inward, so distance from the centre carries degree. `
                        : `Every item here has the same number of edges, so nothing is pulled inward and distance from the centre carries no meaning. `) +
                      `Each line is a pair inside the route's ` +
      ...
      <p className="text-xs leading-relaxed text-white/60">
        ...
        date, then by id, and placed on a ring.{" "}
        {view?.radialDegree
          ? "The most connected are pulled inward, so a mark nearer the centre has more edges."
          : "Every item drawn here has the same number of edges, so the ring is flat and how far a mark sits from the centre means nothing."}{" "}
        The same payload always draws the same picture, so two readings can be compared. A short
        line does not mean two items are near each other in vector space. Only the printed distance
        says that.
      </p>
    </div>
  );
